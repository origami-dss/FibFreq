...
#' Lorentz_fit_periodogram(x1)
#' Lorentz_fit_periodogram(x2)
#' Lorentz_fit_periodogram(x2,  welch_window = TRUE)
#' Lorentz_fit_periodogram(x3)
Lorentz_fit_periodogram <- function(x, delta_t = 1.0, welch_window = FALSE, f_min = 0, f_max = Inf)
{
  ...
  else
  {
    ...
    ww=which.max(Pow)
    argmax_freq <-  freq[ww]
    Lorentzfit <- nls(Pow ~ dcauchy(freq, location, scale),
...
#    res <- list(Lorenz_freq = Lorenz_freq, Lorenz_scale = Lorenz_scale, Lorenz_freq_se = Lorenz_freq_se, Lorenz_scale_se = Lorenz_scale_se)
    res <- list(Lorentz_freq = Lorentz_freq,  Lorentz_scale = Lorentz_scale)
  }
  class(res) <- c("Lorentz_freq", "list")
  return(res)
}
...
lorentz_whittle <- function(x, delta_t = 1.0, welch_window = welch_window, f_min = 0, f_max = Inf)
{
  #
  #   Computes Whittle MLE for the Lorentz fit
  #
  if(!is.vector(x) & !is.numeric(x) & any(is.na(x)) & any(is.infinite(x))) stop("'x' must be non-infinite real-valued numeric vector")
  if (length(x) == 0L) stop("data series to short")
  if (!is.numeric(delta_t) & length(delta_t) != 1L & any(is.infinite(delta_t))) stop("'delta_t' must be finite numeric of length one")
  ...
  P <- Periodogram(x, welch_window = welch_window)
  freq <- P$Frequency
  w <- which(freq > f_min & freq < f_max)
  freq <- freq[w]
  Pow <- P$Power[w]
  PP <- list(Frequency = freq, Power = Pow)
  Res_optim <- optim(c(location = 0.01, scale = 0.02), whittle_loglik, gr=NULL, method = "L-BFGS-B", lower = c(f_min,0.00001), upper = c(f_max, 4.0), P=PP)
...
whittle_loglik <- function(param,  P)
{
  ...
  #  Whittle Log-likelihood
  #
  loglik <- delta_f*(sum(log(Dcauchy)) + sum(Pow/Dcauchy))
  return(loglik)
}
