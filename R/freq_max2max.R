...
# transfer the arguments for splus2R::peaks function
  ...
  if (n_max < 3)
    {
    res <- c(freq_max2max = NA_real_, sd_freq = NA_real_, IQR_freq = NA_real_, n_max = n_max )
    ...
    res <- c(freq_max2max = freq_max2max, sd_freq = freq_max2max_sd, IQR_freq = freq_max2max_IQR, n_max = n_max )
