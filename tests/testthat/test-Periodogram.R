...
test_that("Periodogram throughs an error in case of non-numerical input array x", {
...
test_that("Periodogram throughs an error in case of non-numerical or vector-valued value of delta_t", {
...
test_that("Periodogram throughs an error in case of non-logical value or vector-valued welch_window", {
...
test_that("Periodogram returns an array of dim (length(input/2),2 )", {
  ...
  expect_length(Periodogram(1:10)[1,], 2)
...
test_that("Periodogram returns correct output", {
  ...
  expect_equal(Periodogram(rep(c(1,-1),5))[,2],  c(0,0,0,0,2))
})
