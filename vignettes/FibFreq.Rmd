...
freq_M2M <- freq_max2max(x, delta_t = delta_t)
...
freq_MaxP <- freq_argmax_periodogram(x, delta_t = delta_t)
...
freq_MaxP <- freq_argmax_periodogram(x,
                                     ...
                                     f_min = 1,
                                     f_max = 45)
freq_AVP <- freq_adapted_variable_period(x, delta_t = delta_t)
...
tibble_Lorentz_plot %>%
  ggplot(aes(x = Frequency)) +
  geom_linerange( aes(ymin = 0, ymax = Fourier_Amps), col = "steelblue3", linewidth = 1.5) +
